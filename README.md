...
- **输出质量**：标准 1x、精细 2x、超精细 3x、原图级
- **格式**：1x/2x/3x 支持 PNG 或 JPG；原图级固定为 PNG
- **JPG 质量**：70–100 可调
- **透明与背景**：PNG 保留片基投影外围透明区域，JPG 使用中性暖灰背景

单帧原图级使用当前裁切和旋转后的图片分辨率。若所需画布超过浏览器上限，会保留弹窗并提示改用 3x，不会影响整张索引的原图级流式 PNG 功能。
