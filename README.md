...
2. 等待图片加载完成，自动生成索引图。
3. 使用侧栏调整排版、画幅和胶片外观；背景可选淡黄纸底、纯白或高斯模糊。
4. 点击“观片台”或按 `F` 进入沉浸检片模式。移动鼠标在整张索引上使用观片器，滚轮无极调节倍率（1X–60X）；左键单击切换聚焦/非聚焦模式，按 `Esc` 退出。触控设备可点按切换聚焦。
5. 点击“导出索引图”下载 PNG 或 JPG。导出弹窗可保持当前比例，也可选择 A4 竖向（1:√2）或横向（√2:1）；A4 模式会自动优化每行张数并居中扩展画布，不裁切照片。
